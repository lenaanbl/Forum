...
function getConnexion(){
    $connexion = new PDO("mysql:host=localhost;dbname=blog2;charset=UTF8",'root','');
...
function getRoleByIdAdmin($id)
{
    $connexion = getConnexion();
    $query = 'SELECT id,role FROM user WHERE id=:id AND role=2' ;
    $pdo = $connexion->prepare($query);
    $pdo->execute(array(
		'id'=>$id
    ));
...
function getUserLogin($username,$password){
    $connexion = getConnexion();
    $password = encryptPassword($password);
    $query = 'SELECT id,username FROM user WHERE username=:username AND password=:password AND (role=1 OR role=2)';
    $pdo = $connexion->prepare($query);
    $pdo->execute(array(
        'username'=>$username,
        'password'=>$password
